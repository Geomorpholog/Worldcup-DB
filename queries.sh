...
echo "$($PSQL "SELECT DISTINCT name FROM (teams FULL JOIN games ON teams.team_id=games.winner_id) as win FULL JOIN (teams FULL JOIN games ON teams.team_id=games.opponent_id) as opp  USING(name,year,round) WHERE year=2014 AND round='Eighth-Final';")"
...
echo "$($PSQL "SELECT DISTINCT name FROM (teams FULL JOIN games ON teams.team_id=games.winner_id) as win FULL JOIN (teams FULL JOIN games ON teams.team_id=games.opponent_id) as opp  USING(name,year,round) WHERE name LIKE 'Co%';")"
